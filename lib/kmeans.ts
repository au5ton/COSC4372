...
function getLabels(dataSet, centroids) {
  ...
  for (let i = 0; i < dataSet.length; i++) {
    ...
    labels2[i] = closestCentroidIndex;
  }
...
function kmeans(dataset: any[], k: number, useNaiveSharding = true) {
  if (dataset.length && dataset[0].length && dataset.length > k) {
    // Initialize book keeping variables
    let iterations = 0;
    let oldCentroids, labels, centroids, labels2;
    ...
    while (!shouldStop(oldCentroids, centroids, iterations)) {
      ...
      let temp = getLabels(dataset, centroids);
      labels = temp[0]
      labels2 = temp[1]
